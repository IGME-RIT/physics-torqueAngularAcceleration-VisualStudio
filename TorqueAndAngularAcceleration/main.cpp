...
/*
While computing, storing the the orientation in a vector with the angles along
the three axis can lead to inefficiency as we would have to recalculate the rotation
matrix for the current position and then calculate the displacement matrix and
multiplying them would result in the final position.Thus it is more efficient
to store the current orientation in the form of a matrix.
*/
struct Circle{
	glm::vec3 origin;
	glm::mat4 rotation;
...
void setup()
{
	...
	circle.rotation = glm::mat4(1.0f);
	circle.angularVelocity = glm::vec3(0.0f, 0.0f, 0.0f);
	circle.radius = 0.15f;
	circle.mass = 1.0f;
	circle.angularAcceleration = glm::vec3(0.0f, 0.0f, 0.0f);
	float radius = circle.radius;
	float sideSq = radius * radius * 2;
	circle.I = circle.mass * sideSq / 6.0f;
	...
	for (int i = 0; i < NumberOfDivisions; i++)
	{
		...
		vertices.push_back(VertexFormat(glm::vec3(radius * cos(glm::radians((i + 1)*theta)), radius * sin(glm::radians((i + 1)*theta)), 0.0f), glm::vec4(0.0f, 0.20f, 0.7f, 1.0f)));
	}
...
void update()
{
	// Update the angular velocity using the current acceleration
	circle.angularVelocity = AcceleratedVel(circle.angularAcceleration, circle.angularVelocity, timestep);
	// Set the angular acceleration to 0 once the velocity has been updated.
	// The acceleration is done only 1 frame
	circle.angularAcceleration = glm::vec3(0.0f, 0.0f, 0.0f);
	
	// This "if" statement is necessary because glm::rotate function return invalid matrix for rotaion along 0,0,0 
	// Here we check if the angular velocity is not zero. 
	if (glm::length(circle.angularVelocity) > FLT_EPSILON)
		circle.rotation *= glm::rotate(glm::length(circle.angularVelocity) * timestep, circle.angularVelocity);
	// We are using the angular velocity vector's magnitude as the speed of rotation, and the vector itself as the axis of rotation.
	// multiply the resultant matrix with the current rotation matrix., this gives us the final orientation.
	
	glm::mat4 translation = glm::translate(glm::vec3(circle.origin));
	circle.MVP = PV *(translation*circle.rotation);
}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	{
		circle.angularAcceleration += acc;
		// Increase the acceleration
	}
	if (key == GLFW_KEY_F && (action == GLFW_PRESS || action == GLFW_REPEAT))
	{
		circle.angularAcceleration += force / circle.I;
		// Wafter we get torque, we have to divide it by the moment of Inertia to get the angular acceleration. 
	}
	if (key == GLFW_KEY_S && (action == GLFW_PRESS))
	{
		circle.angularVelocity = glm::vec3(0.0f, 0.0f, 0.0f);
		// STOP the rotation by changing the velocity to 0.
	}
	if (key == GLFW_KEY_SPACE && (action == GLFW_PRESS))
	{
		...
		std::cout << " Acceleration in z-axis (towards you)   : "; std::cin >> acc.z;
		circle.angularAcceleration += acc;

		std::cout << "\n enter the Torque Values: \n";
		std::cout << " Torque in x-axis (horizontal Dir): "; std::cin >> force.x;
		std::cout << " Torque in y-axis (Vertical Dir)  : "; std::cin >> force.y;
		std::cout << " Torque in z-axis (Towards you)   : "; std::cin >> force.z;
		circle.angularAcceleration += force / circle.I;
		...
		std::cout << "\n Keep in mind that since the dimensions are small, the acceleration generates a large amount of acceleration";
	}
}
#pragma endregion
...
void main()
{
	...
	window = glfwCreateWindow(800, 800, "Torques and angular Acceleration", nullptr, nullptr);

	std::cout << "\n This program demonstrates the implementaion of torques and angular acceleration in code\n\n\n\n\n\n\n\n\n\n";
	std::cout << "\n Press and hold \"a\" to apply constant Angular acceleration.";
	std::cout << "\n Press and hold \"f\" to apply constant Torque.";
	std::cout << "\n Press \" Space \" to change values.";
